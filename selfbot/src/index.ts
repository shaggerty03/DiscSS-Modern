...
app.get('/status', async (req, res) => {
    try {
        ...
        if (!channel.isText()) return res.sendStatus(400);
        const response = {
            status: 'OK',
...
const getVideoDetails = async (videoFile: string): Promise<{ fps: number; width: number; height: number; }> => {
...
app.post('/play', async (req, res) => {
    try {
        ...
        if (!channell.isText()) return res.sendStatus(400);
