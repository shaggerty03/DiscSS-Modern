import { getInputMetadata, inputHasAudio, MediaUdp, Streamer, streamLivestreamVideo } from "@dank074/discord-video-stream";

export async function playVideo(video: string | { [key: string]: { [key: string]: string[] } }, udpConn: MediaUdp, season?: number, episode?: number) {
...
export async function playVideoScheduled(video: string, udpConn: MediaUdp, duration: number): Promise<string> {
    let includeAudio = true;
    try {
        const metadata = await getInputMetadata(video);
        includeAudio = inputHasAudio(metadata);
